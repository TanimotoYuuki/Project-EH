	...
	namespace nsSelect
	{
		...
		void RoleSelect::InitColorArrowUIAnimation()
		{
			...
			Vector3 baseColor = { 
				...
				m_arrowUI[enArrowUI_Left].GetMulColor().z 
			};/*元の色。*/
			...
			baseColor = {
				...
				m_arrowUI[enArrowUI_Right].GetMulColor().z
			};/*元の色。*/
		...
		void RoleSelect::UpdateSelect()
		{
			...
			if (g_pad[0]->IsTrigger(enButtonLeft))
			{
				...
				if (m_currentRole == enRole_Sword) { m_currentRole = enRole_Gunner; return; }
			...
			if (g_pad[0]->IsTrigger(enButtonRight))
			{
				...
				if (m_currentRole == enRole_Gunner) { m_currentRole = enRole_Sword; return; }
