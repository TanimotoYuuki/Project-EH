...
namespace nsApp
{
	namespace nsUI
	{
		...
		{
			AddMessage(GetWeaponName(weapon) + L"の" + actionName + L"!", GetPriorityFromActionName(actionName));
		}

		void CommentaryUIManager::AddActionMessage(WeaponType weapon, CommentaryActionType actionType)
		{
			AddMessage(GetWeaponName(weapon) + L"の" + GetActionName(actionType) + L"!", GetPriorityFromActionType(actionType));
		}
		...
		CommentaryMessagePriority CommentaryUIManager::GetPriorityFromActionName(const std::wstring& actionName) const
		{
			...
			if (actionName == L"チャージ攻撃" || actionName == L"いちげき")
				return CommentaryMessagePriority::High;

			if (actionName == L"救助" || actionName == L"復活" || actionName == L"ダウン")
				return CommentaryMessagePriority::Critical;
