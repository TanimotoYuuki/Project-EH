...
namespace nsApp
{
    namespace nsAI
    {
        ...
        {
			/* TSV の personality 列の文字列を BossPersonality に変換。定義されていない文字列なら Balanced を返す。 */
        ...
        {
			/* TSV の BossType 列の文字列を CharacterModelType に変換。定義されていない文字列なら false を返す。 */
        ...
        {
			/* TSV の行から BossTypeParameters を生成。存在しない列はデフォルト値を使用。 */
            BossTypeParameters p;

			/* BossType 列は呼び出し元で処理されているため、ここでは personality 列のみを変換して設定する。 */
        ...
        {
			/* デフォルト値を設定。TSV 読み込みに失敗したときや、TSV に定義されていないボスタイプのパラメータを要求されたときに使用される。 */
            ...
            table[CharacterModelType::TutorialBoss] = { BossPersonality::Defensive,   8.0f, 55.0f, 2.5f, 2.0f, 20.0f, 50, 30, 20, 3000, 0.6f, false, false, 0.8f, 0.1f, false };
        }
    }
}