...
namespace {
	...
	const Vector3 TEXT_UI_INIT_POSITION = { 0.0f,0.0f,0.0f };/*テキストUIの初期位置。*/

	const Vector3 TEXT_UI_INIT_SCALE = { 1.6f,1.6f,1.0f };/*テキストUIの初期大きさ。*/
	...
	const Vector2 AFTER_UI_ANIMATION_SCALE = { 1.25f,1.25f };/*UIの大きさを変えるアニメーション後の透明度。*/
...
namespace nsApp
{
	...
	{
		/*開始処理。*/
		bool GameTimeUpDirection::Start()
		{
			/*スプライトの初期化。*/
			InitSprite();
		...
		/*更新処理。*/
		void GameTimeUpDirection::Update()
		{
		...
		/*描画処理。*/
		void GameTimeUpDirection::Render(RenderContext& rc)
		{
			/*テキストUIの描画。*/
			m_textUI.Draw(rc);
		}
		...
		/*スプライトの初期化。*/
		void GameTimeUpDirection::InitSprite()
		{
			/*テキストUI。*/
			InitTextUI();
		}
		...
		/*テキストUIの初期化。*/
		void GameTimeUpDirection::InitTextUI()
		{
		...
		/*UIアニメーションの初期化。*/
		void GameTimeUpDirection::InitUIAnimation()
		{
			/*UIの大きさを変えるアニメーション。*/
		...
		void GameTimeUpDirection::InitScaleUIAnimation()
		{
			/*UIの透明度を変えるアニメーションの値の設定。*/
			Vector2 baseScale = { m_textUI.GetScale().x,m_textUI.GetScale().y };/*元の大きさ。*/
			Vector2 targetScale = AFTER_UI_ANIMATION_SCALE;/*ターゲットの大きさ。*/

			/*初期化。*/
			m_scaleUIAnimation = std::make_unique<nsApp::nsUI::ScaleUIAnimation>(
		...
		void GameTimeUpDirection::InitAlphaUIAnimation()
		{
			/*UIの透明度を変えるアニメーションの値の設定。*/
		...
		/*UIアニメーションの更新処理。*/
		void GameTimeUpDirection::UpdateUIAnimation()
		{
			if (!m_scaleUIAnimation->IsEnd())
			{
				m_scaleUIAnimation->Update();
		...
		/*スプライトの更新処理。*/
		void GameTimeUpDirection::UpdateSprite()
		{
			m_textUI.Update();/*更新処理。*/
		}
	}
